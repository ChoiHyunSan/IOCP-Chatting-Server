...
***************************/
// - TLS마다 고유하게 MemoryPool을 가지는 형태를 지원하는 클래스.
...
public:
	...
	NLMemoryPool<T>*	GetObjectPool();
	void				RequestFreeList(NLMemoryPool<T>* objectPool);
	DWORD	            GetTotalSize();
private:
	NLMemoryPool<T>*	_objectPoolArr[dfTHREAD_COUNT];
	DWORD				_tlsIndex;
...
{
	NLMemoryPool<T>* objectPool = GetObjectPool();
...
template<typename T>
inline void TLSObjectPool<T>::RequestFreeList(NLMemoryPool<T>* objectPool)
{
	int	maxSize = 0;
	int maxIndex;
...
template<typename T>
inline NLMemoryPool<T>* TLSObjectPool<T>::GetObjectPool()
{
	NLMemoryPool<T>* objectPool = static_cast<NLMemoryPool<T>*>(TlsGetValue(_tlsIndex));
	if (objectPool == nullptr)
	{
		objectPool = new NLMemoryPool<T>(dfINIT_SIZE, false);
