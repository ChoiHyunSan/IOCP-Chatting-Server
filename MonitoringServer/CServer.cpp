...
CServer::CServer(const WCHAR* fileName)
{
	memcpy(_ServerFile, fileName, sizeof(WCHAR) * lstrlenW(fileName));
...
BOOL CServer::ServerSetting()
{
	...
	for (LONGLONG cnt = 0; cnt < _sessionMaxCount; cnt++)
	{
		_emptyIndexQueue.Enqueue(cnt);
	}
...
{
	WParsor parsor;
	BOOL result = parsor.LoadFile(_ServerFile);
	if (result == false)
	{
...
unsigned int CServer::AcceptThreadFunc()
{
	...
	for (;;)
	{
		...
		// result = SetSessionID(newSession);
		// TOOO : SetSessionID 결과값에 대한 예외처리
...
{
	LONGLONG index;
	if (_emptyIndexQueue.Dequeue(index) == false)
	{
...
VOID CServer::InsertIndexToQueue(LONGLONG index)
{
	_emptyIndexQueue.Enqueue(index);
}
