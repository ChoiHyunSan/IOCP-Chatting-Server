...
CMonitor::CMonitor(HANDLE hProcess)
{
	...
	PdhAddCounter(_cpuQuery, L"\\Process(MonitoringServer)\\Private Bytes", NULL, &_processUserMemory);				// BYTE
	PdhAddCounter(_cpuQuery, L"\\Process(MonitoringServer)\\Pool Nonpaged Bytes", NULL, &_processNonpagedMemory);	// BYTE
