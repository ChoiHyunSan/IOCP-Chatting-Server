...
/*
* 
TLS ObjectPool에서 사용하기 위해 락프리가 적용되지 않은 메모리 풀 (NL : No Lock)
...
public:
	NLObjectPool(int iBlockNum, bool bPlacementNew = false);
...
public:
	...
	void	TakeSubList(NLObjectPool<T>* objectPool);
	void	SwapSubList();
...
template<typename T>
inline NLObjectPool<T>::NLObjectPool(int iBlockNum /* 초기 생성 개수 */, bool bPlacementNew /*꺼낼 때 생성자 호출 여부*/)
...
template<typename T>
inline NLObjectPool<T>::~NLObjectPool()
{
...
template<typename T>
inline T* NLObjectPool<T>::Alloc(void)
{
	if (_pFreeNode == nullptr)
	{
		for (int i = 0; i < 300; i++)
...
template<typename T>
inline bool NLObjectPool<T>::Free(T* pData)
{
	st_BLOCK_NODE* topNode = _pFreeNode;
...
template<typename T>
inline void NLObjectPool<T>::FreeToSub(T* pData)
{
...
template<typename T>
inline void NLObjectPool<T>::TakeSubList(NLObjectPool<T>* objectPool)
{
	AcquireSRWLockExclusive(&_subLock);
...
template<typename T>
inline void NLObjectPool<T>::SwapSubList()
{
	AcquireSRWLockExclusive(&_subLock);
